 ...
 * env vars that the remote's ~/.nekofree settings.env MUST NOT clobber (see
 ...
 * user's ~/.nekofree/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.nekofree/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.nekofree.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.nekofree/settings.json env (gh#217). policy/flag
