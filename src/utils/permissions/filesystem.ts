...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.nekofree', 'skills')),
      prefix: '~/.nekofree/skills/',
    },
  ]
 ...
 * Path format: ~/.nekofree/projects/{sanitized-cwd}/...
 */
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // Accepts both the broad patterns ('/.claude/**', '~/.nekofree/**') and
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.nekofree/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.nekofree is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.nekofree/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .claude/ only (not ~/.nekofree/) since launch.json is per-project.
  if (
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.nekofree/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.nekofree/teams/) for swarm coordination
