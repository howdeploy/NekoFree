 ...
 * user-controlled files (~/.nekofree/settings.json and ~/.nekofree.json),
 ...
 *
 * We read from global config (~/.nekofree.json) and user settings
 * (~/.nekofree/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.nekofree/settings.json),
