...
|------|-------|-----|---------|
| \`~/.nekofree/settings.json\` | Global | N/A | Personal preferences for all projects |
...
\`\`\`json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "jq -r '.tool_input.command' >> ~/.nekofree/bash-log.txt"
      }]
    }]
  }
}
...
1. **Check the settings file** - Read ~/.nekofree/settings.json or .claude/settings.json
